...
class test_entity(unittest.TestCase):
    ...
    def test_that_random_walk_changes_direction_of_velocity(self):
        ...
        param_dict = {"walking_speed_Z": 2.0, "smooth_rand_walk": 1}
        #--
        
        sub_direction_before = subject.get_direction()
        subject.random_walk(param_dict)
    ...
    def test_that_random_walk_changes_speed_of_velocity_to_zombi_speed(self):
        ...
        assert speed_after == param_dict["walking_speed_Z"], "entity should have walking_speed_Z speed after walk"
    ...
    def test_that_entiy_makes_a_random_walk_with_smooth_factor(self):
        ...
        for factor in np.arange(0.0, 1.3,0.1):
            ...
            for _ in range(1000):
                ...
                param_dict = {"walking_speed_Z": 2.0, "smooth_rand_walk": factor}
                #--
                #1 get old direction
            ...
            assert min(angles) >= -np.pi * param_dict["smooth_rand_walk"], "random walk angle is outside of einschränkung"
            assert max(angles) <= np.pi  * param_dict["smooth_rand_walk"], "random walk angle is outside of einschränkung"
            if factor == 0.0:
    ...
    def test_zombie_chases_human_when_alerted(self):
        ...
        # Action
        subject.zombie_walk( [] ,param_dict)
    ...
    def test_zombie_wanders_when_not_alerted(self):
        ...
        params = {"awareness_r_Z": 10.0, "smooth_rand_walk": 0.5}
        

        # Assert that random_walk was the fallback behavior
        subject.random_walk.assert_called_once_with(params)
...
def make_subject(pos, velocity, heap_entries):
    ...
    # 2. Provide the logic for speed
    subject.get_speed = lambda: entity.get_speed(subject)

    # 3. Bind the random_walk method from the real class
    # This allows the mock to execute the REAL code using the mock's data
    subject.random_walk = lambda params: entity.random_walk(subject, params)
    subject.zombie_walk = lambda entities, params: entity.zombie_walk(subject, entities, params)
    subject.update_alerted_state_alerter_pos = lambda ents, r: \
        entity.update_alerted_state_alerter_pos(subject, ents, r)
    ...
    def update_vel(new_v):
        subject.velocity = new_v
        
    subject.change_velocity = update_vel
